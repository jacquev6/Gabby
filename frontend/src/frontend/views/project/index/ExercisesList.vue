...
<template>
  <template v-if="project.relationships.textbooks.length || project.relationships.exercises.length">
    ...
    <template v-for="textbook in project.relationships.textbooks">
      <h3><RouterLink :to="{name: 'project-textbook-page-list-exercises', params: {projectId: project.ud, textbookId: textbook.id, page: 1}}">{{ textbook.attributes.title }}</RouterLink><template v-if="textbook.attributes.publisher">, {{ textbook.attributes.publisher }}</template><template v-if="textbook.attributes.year"> ({{ textbook.attributes.year }})</template></h3>
      <template v-if="exercisesByTextbookAndPage[textbook.id]">
        <ul v-for="[page, exercises] of Object.entries(exercisesByTextbookAndPage[textbook.id]?.pages)">
          <li>
            <RouterLink :to="{name: 'project-textbook-page-list-exercises', params: {projectId: project.ud, textbookId: textbook.id, page}}">Page {{ page }}</RouterLink>
            <ul>
