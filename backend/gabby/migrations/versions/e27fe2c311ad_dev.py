...
def upgrade():
    ...
    op.alter_column("exercises", "instructions", new_column_name="instructions_text")
    op.alter_column("exercises", "wording", new_column_name="wording_text")
    op.alter_column("exercises", "example", new_column_name="example_text")
    op.alter_column("exercises", "clue", new_column_name="clue_text")
    ...
    with orm.Session(op.get_bind()) as session:
        for i, exercise in enumerate(session.query(Exercise).all()):
            ...
            # Fix instructions, wording, etc. (these assignments trigger side effects)
            exercise.instructions = exercise._instructions_text + "\n"
            exercise.wording = exercise._wording_text + "\n"
            exercise.example = exercise._example_text + "\n"
            exercise.clue = exercise._clue_text + "\n"

            # Check
            (adapted, delta) = exercise.make_adapted_and_delta()
            assert delta.instructions == exercise.instructions_deltas
            assert delta.wording == exercise.wording_deltas
            assert delta.example == exercise.example_deltas
            assert delta.clue == exercise.clue_deltas
