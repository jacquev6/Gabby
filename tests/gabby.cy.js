describe('Gabby', () => {
  ...
  it('performs extraction from scratch', () => {
    ...
    canvas.screenshot('project-textbook-page/create-exercise-tracing-bounding-rectangle', {clip: {x: 0, y: 0, width: 1000, height: 200}})
    canvas.trigger('pointerup', 140, 105, { pointerId: 1 })

    cy.screenshot('project-textbook-page/create-exercise', {clip: {x: 0, y: 50, width: 575, height: 750}})
    ...
    canvas.screenshot('project-textbook-page/selecting-in-pdf', {clip: {x: 0, y: 0, width: 190, height: 75}})
    canvas.trigger('pointerup', 135, 55, { pointerId: 1 })
    cy.screenshot('project-textbook-page/selected-in-pdf', {clip: {x: 0, y: 0, width: 500, height: 660}})
  ...
  it('loads existing data', () => {
    ...
    cy.screenshot('index/index', {clip: {x: 0, y: 0, width: 1000, height: 400}})

    cy.get('a:contains("Premier projet de test")').click()

    cy.get('div.busy').should('not.exist')
    cy.screenshot('project/project', {clip: {x: 0, y: 0, width: 1000, height: 400}})
    ...
    cy.screenshot('project-textbook-page/modify-exercise', {clip: {x: 0, y: 50, width: 575, height: 1000}})

    cy.get('label:contains("Type d\'adaptation")').next().select('selectThingsAdaptation')
    cy.get('div.busy').should('not.exist')
    cy.screenshot('project-textbook-page/three-columns', {clip: {x: 0, y: 0, width: 1000, height: 330}})
